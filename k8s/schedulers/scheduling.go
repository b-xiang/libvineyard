...
import (
	...
	v1beta1 "github.com/alibaba/libvineyard/k8s/api/k8s/v1beta1"
	clientset "github.com/alibaba/libvineyard/k8s/generated/clientset/versioned"
	clientsetv1beta1 "github.com/alibaba/libvineyard/k8s/generated/clientset/versioned/typed/k8s/v1beta1"
)
...
func (vs *VineyardScheduling) Score(ctx context.Context, state *framework.CycleState, pod *v1.Pod, nodeName string) (int64, *framework.Status) {
	...
	klog.Infof("scoring for pod %v on node %v", GetNamespacedName(pod), nodeName)
	...
	klog.Infof("scoring for pod of job %v, with %v replicas (rank %v), and requires %v", job, replica, rank, requires)
...
func (vs *VineyardScheduling) PostBind(ctx context.Context, _ *framework.CycleState, pod *v1.Pod, nodeName string) {
	klog.Infof("bind pod %v on node %v", GetNamespacedName(pod), nodeName)
	job, replica, requires, err := vs.GetVineyardLabels(pod)
	if err != nil {
		// ignore: might not be a vineyard job
		return
	}
	klog.Infof("bind pod of job %v, with %v replicas, and requires %v", job, replica, requires)
